...
# Sums phi along the columns (k) for each row (i)
function cpu_inte!(phi, psi, N)
    @threads for i in 1:N
        sum_val = 0.0
        # Replicating the specific summation loop order
        for k in 1:N
...
# This was serial (threads=1) in CUDA, so it is serial here.
function cpu_spline_coeffs!(y, alp, b, c, d, scratch, N)
    ...
    # Copy out
    # Parallelizing this copy is fine, but overhead might outweigh benefit for small N
...
# 1D Convolution with boundary handling
function cpu_conv!(psi, I, convKernel, M, kernelN, subDiv, IZ)
    halo = (kernelN - 1) ÷ 2
    
    @threads for gid in 1:M
        acc = 0.0
        # Replicating the sliding window logic
        # Corresponds to: s_psi[tid + k - 1] where s_psi is centered
        for k in 1:kernelN
            # Calculate global index of the neighbor
...
function cpu_compute_flux!(phi, J, K, percoll, R, I, gradWing, p::SimParams)
    # Julia is column-major. Iterating j (cols) then i (rows) is cache friendly.
    # However, logic inside uses i, j indices.
    @threads for j in 1:p.N
        for i in 1:p.N
...
function init_phi!(phi, R, N, wingL, IZ, IR, PSI)
    ...
    # We can write directly to the array now, no GPU staging needed
    fill!(phi, 0.0)
    
    # Parallel initialization
    @threads for j in 0:N-1
        for i in 0:N-1
            zVal = i * IZ; rVal = j * IR
...
function main()
    ...
    outDir = "sim_cpu_$timestamp/"
    mkpath(outDir)
    
    println("\n============================================================")
    println("   julia cpu simulation: conservative finite volume transport    ")
    println("============================================================")
    ...
    if !isempty(step_times)
        ...
        println("  -> statistics based on $n_meas pure computation steps (CPU Wall Time)")
    else
