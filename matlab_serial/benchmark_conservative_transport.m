function benchmark_conservative_transport()
% BENCHMARK_CONSERVATIVE_TRANSPORT
% Runs a benchmark suite on run_conservative_transport.m matching the 
% parameters and logic of the provided Python/C++ benchmark script.

    % --- Configuration ---
    output_file = 'benchmark_matlab_results.tsv';
    
    % Fixed Parameters (matching Python script)
    isf_val = 0.00001;
    psi_val = 0.02;
    it_val  = 0.001;
    ...
    % Python script had range(13, 14) which is just 8192, but comment said 128-8192.
    % We enable the full range here.
    grid_powers = 13:13; 
    ...
    fprintf('=======================================================\n');
    fprintf(' Benchmarking MATLAB: run_conservative_transport.m\n');
    fprintf('=======================================================\n');
    ...
    for k = 1:length(grid_sizes)
        ...
        % Logic matches python: t_val = 15360.0 / float(n)
        t_val = 15360.0 / double(n) / 12;
        n_steps = floor(t_val / it_val);
        
        % Prepare arguments for run_conservative_transport
        % Signature: (N, [ISF], [PSI], [IT], [T], [NO], [D_coeff])
        
        try
            % We use evalc to capture the standard output (stdout) 
            % This allows us to parse the results without modifying the core script
            cmd = sprintf('run_conservative_transport(%d, %.10f, %.10f, %.10f, %.10f, %d, %.10f);', ...
            ...
            % Pattern matches the specific print format in run_conservative_transport.m
